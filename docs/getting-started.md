...
The lockfile is authoritative. `npm ci` should be preferred for repeatable
installs; the `--legacy-peer-deps` flag is required by the pinned Jupiter SDK
dependency graph. The original dependency-install issue and its exact cause
